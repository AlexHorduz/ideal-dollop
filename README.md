...
## Вибір моделей
* YOLO одна з де-факто стандартних моделей для object detection
  * має готові натреновані ваги на великих датасетах
  * має "екосистему" інструментів для тренування, тюнінгу, інференсу
* RFDetr сучасна трансформерна модель на основі DETR-підходу
  * використовує attention-механізм
  * якість локалізації та кращу роботу з перекриттями об’єктів
  
## Результати моделей
* YOLO
  * Швидко сходиться під час тренування, стабільна крива втрат
  * Показує високу precision для добре контрастних та типових знаків
  * Гірше працює на дрібних знаках та знаках із частковими перекриттями
* RFDetr
  * Тренування дещо повільніше та чутливіше до гіперпараметрів
  * Крива втрат показує стабільне покращення при довшому донавчанні
  * mAP по класах зазвичай вищий для сцен з кількома знаками, перекриттями чи складним фоном, модель краще розуміє контекст навколо знаків
  
YOLO краще підходить для швидких, простих сценаріїв де один-два знаки, хороше освітлення, невелика відстань. RFDetr демонструє більш високу якість на складних сценах, зокрема при багатьох об’єктах, перекриттях та зашумленому фоні

## Обрані методи
* Guided Backprop метод дозволяє побачити, які саме пікселі та контури найбільше впливають на вихід моделі
  * Для YOLO метод показує чітку орієнтацію на контури знаків та контрастні межі між знаком і фоном. Модель сильно фокусується на яскравих елементах
  * Для RFDetr метод демонструє не тільки контури самого знака, але й додаткові елементи контексту, що підтверджує використання глобального контексту
* Grad-CAM дозволяє зрозуміти, в якій області зображення модель бачит потрібний дорожній знак
  * Для YOLO активні області добре співпадають з реальними боксами знаків, проте видно що теплова карта зміщена у зоні об’єкта, що вказує на слабкі фічі для цього прикладу
  * Для RFDetr метод Grad-CAM показує більш розширені області активації: модель захоплює не тільки знак, а й навколишній контекст, проте тут видно що модель іноді "розпиляється" на зайві області з подібною текстурою

## Висновки
YOLO швидка, ефективна модель з добре відпрацьованою архітектурою для реального часу.
YOLO краще працює на простих сценах та середніх/великих знаках, демонструючи високу точність і швидкість

RFDetr сучасний трансформерний підхід, який краще використовує контекст сцени та потенційно забезпечує вищу якість детекції на складних прикладах
RFDetr краще відпрацьовує складні сцени, дрібні та перекриті знаки, але інколи страждає від хибнопозитивних прогнозів та вимагає більшого обчислювального ресурсу

Інтерпретаційні методи Guided Backprop та Grad-CAM:
* Підтвердили, що обидві моделі дійсно спираються на релевантні області зображення , а саме контури та область знака
* Показали різницю в стилі моделей: YOLO більш локальна і фокусується на контрастних контурах, тоді як RFDetr ширше враховує контекст сцени.
* Виявилии типові причини помилок такі як малі об’єкти, слабкий контраст, фон з подібною текстурою, що може бути використано для подальшого покращення датасету та аугментацій.
